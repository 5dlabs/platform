...
# GitHub Actions Run Cleanup Script (Minutes-based)
# Deletes workflow runs older than specified minutes
...
Clean up old GitHub Actions workflow runs (minutes-based)

OPTIONS:
    -r, --repo REPO         Repository in format 'owner/repo' (default: $REPO)
    -m, --minutes MINUTES   Keep runs from last N minutes (default: $MINUTES_TO_KEEP)
...
EXAMPLES:
    $0 --minutes 30                          # Keep last 30 minutes
    $0 --minutes 5 --workflow build-rust-image.yml  # Clean specific workflow, keep 5 minutes
    $0 --status failure --minutes 60         # Clean only failed runs older than 60 minutes
    $0 --keep-successful --minutes 15        # Keep successful runs, delete failed ones older than 15 minutes
    $0 --dry-run --minutes 15                # Preview what would be deleted
    $0 --force --minutes 0                   # Delete ALL runs without confirmation
...
# Calculate cutoff date (minutes ago)
if [ "$MINUTES_TO_KEEP" -eq 0 ]; then
    CUTOFF_DATE="9999-12-31T23:59:59Z"  # Far future date to delete everything
    CUTOFF_DISPLAY="ALL RUNS"
else
    # Handle both macOS and Linux date commands for minutes
    if date -v-1d >/dev/null 2>&1; then
        # macOS date command
        CUTOFF_DATE=$(date -u -v-${MINUTES_TO_KEEP}M +%Y-%m-%dT%H:%M:%SZ)
    else
        # Linux date command
        CUTOFF_DATE=$(date -u -d "$MINUTES_TO_KEEP minutes ago" +%Y-%m-%dT%H:%M:%SZ)
    fi
    CUTOFF_DISPLAY="runs older than $MINUTES_TO_KEEP minutes (before $CUTOFF_DATE)"
fi

echo -e "${BLUE}GitHub Actions Cleanup Script (Minutes-based)${NC}"
echo "=============================================="
...
format_run_info() {
    ...
    if date -v-1d >/dev/null 2>&1; then
        # macOS date command
        date_display=$(date -j -f "%Y-%m-%dT%H:%M:%SZ" "$created_at" +"%Y-%m-%d %H:%M:%S" 2>/dev/null || echo "$created_at")
    else
        # Linux date command
        date_display=$(date -d "$created_at" +"%Y-%m-%d %H:%M:%S" 2>/dev/null || echo "$created_at")
    fi
